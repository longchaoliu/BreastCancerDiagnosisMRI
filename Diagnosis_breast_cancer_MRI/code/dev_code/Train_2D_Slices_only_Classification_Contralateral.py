...
NAME = 'Contra_RandomSlices_DataAug_classifier_train36032_val4038_DataAug_Clinical_Contra_depth6_filters42_L21e-05_batchsize8'

CNN = True
RESNET = False
IMAGENET_WEIGHTS = False
MODEL_NAME = 'Contra_RandomSlices_DataAug_classifier_train36032_val4038_DataAug_Clinical_Contra_depth6_filters42_L21e-05_batchsize8'

LOAD_PRETRAINED_MODEL = True
PRETRAINED_MODEL_WEIGHTS = '/home/deeperthought/Projects/DGNS/2D_Diagnosis_model/Sessions/Contra_RandomSlices_DataAug_classifier_train36032_val4038_DataAug_Clinical_Contra_depth6_filters42_L21e-05_batchsize8/last_model_weights.npy'
...
if LOAD_PRESAVED_DATA:
    ...
    if USE_CONTRALATERAL:
        available_data = os.listdir(DATA_PATH.replace('/X','/Contra'))
    ...
    unsegmented_malignants = MASTER.loc[(MASTER['Pathology'] == 'Malignant')*(MASTER['Segmented'] == 0), 'Scan_ID'].values
    
    available_data_scanIDs = [x for x in available_data_scanIDs if x not in unsegmented_malignants]
    MASTER = MASTER.loc[MASTER['Scan_ID'].isin(available_data_scanIDs)]
...
#partition.keys()
#
#not_in_labels = [ x for x in partition['train'] if x not in labels.keys()]
#
#not_in_labels = list(set([x[:31] for x in partition['train']]) - set(labels.keys()))
#not_in_labels = list(set([x[:31] for x in partition['validation']]) - set(labels.keys()))
#not_in_labels = list(set([x[:31] for x in partition['train']]) - set(labels.keys()))

partition['train'] = [x for x in partition['train'] if x[:31] in labels.keys()]
partition['validation'] = [x for x in partition['validation'] if x[:31] in labels.keys()]
partition['test'] = [x for x in partition['test'] if x[:31] in labels.keys()]
...
available_data = os.listdir(DATA_PATH)
remove = list(set(partition['train']) - set(available_data))
print('Removing {} train images that are not present in Data path'.format(len(remove)))
partition['train'] = list(set(partition['train']) - set(remove))

remove = list(set(partition['validation']) - set(available_data))
print('Removing {} validation images that are not present in Data path'.format(len(remove)))
partition['validation'] = list(set(partition['validation']) - set(remove))

remove = list(set(partition['test']) - set(available_data))
print('Removing {} test images that are not present in Data path'.format(len(remove)))
partition['test'] = list(set(partition['test']) - set(remove))
...
if PREDICTION_ONLY:
    ...
    get_results_on_dataset(best_model, scans_validation, labels,  MASTER, NAME, OUTPUT_PATH, USE_CLINICAL, USE_CONTRALATERAL, USE_PREVIOUS, clinical_features)
