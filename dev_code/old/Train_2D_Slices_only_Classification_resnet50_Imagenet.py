...
# -*- coding: utf-8 -*-
"""
Created on Fri May 26 12:39:26 2023
...
NAME = 'ResNet50_ImageNet_classifier_train52598_val5892_DataAug_Clinical_depth6_filters42_L21e-05_batchsize8'

RESNET = True
IMAGENET_WEIGHTS = True
MODEL_NAME = 'ResNet50_ImageNet_classifier_train52598_val5892_DataAug_Clinical_depth6_filters42_L21e-05_batchsize8'

LOAD_PRETRAINED_MODEL = True
PRETRAINED_MODEL_WEIGHTS = '/home/deeperthought/Projects/DGNS/2D_Diagnosis_model/Sessions/ResNet50_ImageNet_classifier_train52598_val5892_DataAug_Clinical_depth6_filters42_L21e-05_batchsize8/last_model_weights.npy'
...
def load_and_preprocess(all_subject_channels, T1_pre_nii_path):
    ...
    if t1post.shape[1] != 512:
        ...
        t1post = resize(t1post, output_shape=output_shape, preserve_range=True, anti_aliasing=True, mode='constant')
        slope1 = resize(slope1, output_shape=output_shape, preserve_range=True, anti_aliasing=True, mode='constant')
        slope2 = resize(slope2, output_shape=output_shape, preserve_range=True, anti_aliasing=True, mode='constant')
...
def preprocess_data_imagenet(numpy_array):
    # min max norm for uint8 and 0-255
    numpy_array[:,:,:,0] = 255.*(numpy_array[:,:,:,0] - numpy_array[:,:,:,0].min() )/ (numpy_array[:,:,:,0].max() - numpy_array[:,:,:,0].min() )
    numpy_array[:,:,:,1] = 255.*(numpy_array[:,:,:,1] - numpy_array[:,:,:,1].min() )/ (numpy_array[:,:,:,1].max() - numpy_array[:,:,:,1].min() )
    numpy_array[:,:,:,2] = 255.*(numpy_array[:,:,:,2] - numpy_array[:,:,:,2].min() )/ (numpy_array[:,:,:,2].max() - numpy_array[:,:,:,2].min() )
        
    img = resize(numpy_array, output_shape=(1,224,224,3), anti_aliasing=True, preserve_range=True, mode='constant')
    img = np.array(img, dtype='uint8')

    resnet_input = tf.keras.applications.resnet50.preprocess_input(img, data_format=None)
    
    return resnet_input



def make_prediction_whole_scan(model, all_data, clinic_info_exam, USE_CONTRALATERAL, USE_PREVIOUS, RESNET50_IMAGENET):
    ...
    for i in range(t1post.shape[0]):
        ...
        if RESNET50_IMAGENET: 
                X = preprocess_data_imagenet(X)
...
def get_results_on_dataset(model, scans_list, labels, Data_description, NAME, OUT, USE_CLINICAL, USE_CONTRALATERAL, USE_PREVIOUS, clinical_info, RESNET50_IMAGENET=False, name='VAL'):
    ...
    for scan in scans_list:
        ...
        slice_preds = make_prediction_whole_scan(model, all_data[2:], clinic_info_exam, USE_CONTRALATERAL, USE_PREVIOUS, RESNET50_IMAGENET)
...
class DataGenerator_classifier(tf.keras.utils.Sequence): # inheriting from Sequence allows for multiprocessing functionalities
    ...
    def __init__(self, list_IDs,labels,clinical_info, data_path='', batch_size=4, dim=(512,512), n_channels=3,
                 ...
                 use_contralateral=False, use_previous=False, resnet50_imagenet=False, data_description = ''):
        'Initialization'
        self.dim = dim
        ...
        self.resnet50_imagenet = resnet50_imagenet
    ...
    def preprocess_imagenet(self, path_to_file):
        np_frame = np.load(path_to_file, allow_pickle=True)
        # min max norm for uint8 and 0-255
        np_frame[:,:,:,0] = 255.*(np_frame[:,:,:,0] - np_frame[:,:,:,0].min() )/ (np_frame[:,:,:,0].max() - np_frame[:,:,:,0].min() )
        np_frame[:,:,:,1] = 255.*(np_frame[:,:,:,1] - np_frame[:,:,:,1].min() )/ (np_frame[:,:,:,1].max() - np_frame[:,:,:,1].min() )
        np_frame[:,:,:,2] = 255.*(np_frame[:,:,:,2] - np_frame[:,:,:,2].min() )/ (np_frame[:,:,:,2].max() - np_frame[:,:,:,2].min() )
            
        img = resize(np_frame, output_shape=(1,224,224,3), anti_aliasing=True, preserve_range=True, mode='constant')
        img = np.array(img, dtype='uint8')
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):   
            ...
            if self.resnet50_imagenet:
                X[i,:,:,:3] = self.preprocess_imagenet(self.data_path + ID)
            else:
...
params_train = {'dim': INPUT_DIMS,
          ...
          'resnet50_imagenet':IMAGENET_WEIGHTS,
          'data_description':MASTER}

params_val = {'dim': INPUT_DIMS,
          'data_path': DATA_PATH,
          'batch_size': BATCH_SIZE,
          'n_classes': 2,
          'n_channels': CHANNELS,
...
if PREDICTION_ONLY:
    ...
    for set_name in SETS_FOR_PREDICTION:
        ...
        get_results_on_dataset(model, scans_test, labels,  MASTER, NAME, OUTPUT_PATH, USE_CLINICAL, 
                               USE_CONTRALATERAL, USE_PREVIOUS, clinical_features,IMAGENET_WEIGHTS, set_name)    
    ...
    get_results_on_dataset(model, scans_validation, labels,  MASTER, NAME, OUTPUT_PATH, USE_CLINICAL, USE_CONTRALATERAL, USE_PREVIOUS, IMAGENET_WEIGHTS, clinical_features)
