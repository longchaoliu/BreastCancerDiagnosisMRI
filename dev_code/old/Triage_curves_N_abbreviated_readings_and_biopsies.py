...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan  9 16:24:36 2024
...
RESULTS_SLICE_DISTANCES = '/home/deeperthought/Projects/DGNS/2D_Diagnosis_model/Sessions/FullData_RandomSlices_DataAug__classifier_train52598_val5892_DataAug_Clinical_depth6_filters42_L21e-05_batchsize8/test_result_with_deltas.csv'
...
def bootstrap_triage_curves(results, n_draws, n_steps):
    ...
    for i in range(DRAWS):
        ...
        TMP = results.sample(replace=True, n=len(results))
        if len(TMP.loc[TMP['y_true'] == 1]) == 0:
            continue
...
results_slices = pd.read_csv(RESULTS_SLICE_DISTANCES)

results_slices['min_distance'].value_counts()


plt.hist(results.loc[results['y_true']==0,'y_pred'], color='g', bins=100, alpha=0.5)

plt.hist(results_slices.loc[(results_slices['y_true']==1)*(results_slices['min_distance'] == 0),'y_pred'], color='darkred',  bins=100, label='0', alpha=1)
plt.hist(results_slices.loc[(results_slices['y_true']==1)*(results_slices['min_distance'] == 1),'y_pred'], color='red', bins=100, label='1', alpha=1)
# plt.hist(results_slices.loc[(results_slices['y_true']==1)*(results_slices['min_distance'] == 2),'y_pred'], color='darkorange', bins=100, label='2', alpha=1)
plt.hist(results_slices.loc[(results_slices['y_true']==1)*(results_slices['min_distance'] == 3),'y_pred'], color='orange', bins=100, label='3', alpha=1)
plt.hist(results_slices.loc[(results_slices['y_true']==1)*(results_slices['min_distance'] == 4),'y_pred'], color='yellow', bins=100, label='4', alpha=1)
plt.hist(results_slices.loc[(results_slices['y_true']==1)*(results_slices['min_distance'] == 5),'y_pred'], color='gray', bins=100, label='5', alpha=1)

plt.vlines(x=results_slices.loc[(results_slices['y_true']==1)*(results_slices['min_distance'] == 0),'y_pred'].min(),ymin=0,ymax=100,label='0', color='darkred', linestyle='--', linewidth=1)
plt.vlines(x=results_slices.loc[(results_slices['y_true']==1)*(results_slices['min_distance'] == 1),'y_pred'].min(),ymin=0,ymax=100,label='1', color='red', linestyle='--', linewidth=1)
# plt.vlines(x=results_slices.loc[(results_slices['y_true']==1)*(results_slices['min_distance'] == 2),'y_pred'].min(),ymin=0,ymax=100,label='2', color='darkorange', linestyle='--', linewidth=1)
plt.vlines(x=results_slices.loc[(results_slices['y_true']==1)*(results_slices['min_distance'] == 3),'y_pred'].min(),ymin=0,ymax=100,label='3', color='orange', linestyle='--', linewidth=1)
plt.vlines(x=results_slices.loc[(results_slices['y_true']==1)*(results_slices['min_distance'] == 4),'y_pred'].min(),ymin=0,ymax=100,label='4', color='yellow', linestyle='--', linewidth=1)
plt.vlines(x=results_slices.loc[(results_slices['y_true']==1)*(results_slices['min_distance'] == 5),'y_pred'].min(),ymin=0,ymax=100,label='5', color='gray', linestyle='--', linewidth=1)

plt.yscale('log')
plt.legend()

plt.hist(results_slices.loc[(results_slices['y_true']==1)*(results_slices['min_distance'] < 1),'y_pred'], color='r', bins=100)
plt.hist(results.loc[results['y_true']==0,'y_pred'], color='g', bins=100, alpha=0.5)
plt.yscale('log')
...
# plot_triage_curve(triage_curves_misses, xlabel='Effort (1 - abbreviated readings)', ylim=[0.0,1.05], path='', figsize=(4,4))#path=PATH+'Effort_abbreviated_readings.png')
...
resB = results.loc[results['y_true'] == 0]
misses = resM.loc[resM['min_distance'] >= DELTA]
result_misses = pd.concat([resB, misses])

triage_curves_misses_delta0 = bootstrap_triage_curves(result_misses, n_draws=N_DRAWS, n_steps=N_STEPS)

triage_0abbreviated_readings_plot = plot_triage_curve(triage_curves_misses_delta0, xlabel='Effort ({} - abbreviated readings)'.format(DELTA), path='', figsize=(4,4))#path=PATH+'Effort_abbreviated_readings.png')
...
triage_curves_misses_delta1 = bootstrap_triage_curves(result_misses, n_draws=N_DRAWS, n_steps=N_STEPS)

triage_1abbreviated_readings_plot = plot_triage_curve(triage_curves_misses_delta1, xlabel='Effort ({} - abbreviated readings)'.format(DELTA), path='', figsize=(4,4))#path=PATH+'Effort_abbreviated_readings.png')

#%% Triage curves Miss := delta > 1
...
triage_curves_misses_delta2 = bootstrap_triage_curves(result_misses, n_draws=N_DRAWS, n_steps=N_STEPS)

triage_2abbreviated_readings_plot = plot_triage_curve(triage_curves_misses_delta2, xlabel='Effort ({} - abbreviated readings)'.format(DELTA), ylim=[0,1], xlim=[0,1], path='', figsize=(4,4))#path=PATH+'Effort_abbreviated_readings.png')

#%% Triage curves Miss := delta > 2
...
triage_curves_misses_delta3 = bootstrap_triage_curves(result_misses, n_draws=N_DRAWS, n_steps=N_STEPS)

triage_3abbreviated_readings_plot = plot_triage_curve(triage_curves_misses_delta3, xlabel='Effort ({} - abbreviated readings)'.format(DELTA),  path='', figsize=(4,4))#path=PATH+'Effort_abbreviated_readings.png')
...
triage_curves_misses_delta4 = bootstrap_triage_curves(result_misses, n_draws=N_DRAWS, n_steps=N_STEPS)

triage_4abbreviated_readings_plot = plot_triage_curve(triage_curves_misses_delta4, xlabel='Effort ({} - abbreviated readings)'.format(DELTA), path='', figsize=(4,4))#path=PATH+'Effort_abbreviated_readings.png')
...
triage_curves_misses_delta5 = bootstrap_triage_curves(result_misses, n_draws=N_DRAWS, n_steps=N_STEPS)

triage_5abbreviated_readings_plot = plot_triage_curve(triage_curves_misses_delta5, xlabel='Effort ({} - abbreviated readings)'.format(DELTA), path='', figsize=(4,4))#path=PATH+'Effort_abbreviated_readings.png')
...
MASTER = MASTER.loc[MASTER['BIRADS'].isin(['1','2','3','4','5','6'])]

results = pd.read_csv(RESULTS_TABLE_PATH)
results_slices = pd.read_csv(RESULTS_SLICE_DISTANCES)

results_slices = pd.merge(results_slices, MASTER[['Scan_ID','BIRADS']], left_on='scan', right_on='Scan_ID')
results = pd.merge(results, MASTER[['Scan_ID','BIRADS']], left_on='scan', right_on='Scan_ID')

results_slices['min_distance'].value_counts()


plt.hist(results.loc[(results['BIRADS'].isin(['4','5']))*(results['y_true']==0),'y_pred'], color='g', bins=100, alpha=0.5)

plt.hist(results_slices.loc[(results_slices['BIRADS'].isin(['4','5']))*(results_slices['y_true']==1)*(results_slices['min_distance'] == 0),'y_pred'], color='darkred',  bins=100, label='0', alpha=1)
plt.hist(results_slices.loc[(results_slices['BIRADS'].isin(['4','5']))*(results_slices['y_true']==1)*(results_slices['min_distance'] == 1),'y_pred'], color='red', bins=100, label='1', alpha=1)
# plt.hist(results_slices.loc[(results_slices['y_true']==1)*(results_slices['min_distance'] == 2),'y_pred'], color='darkorange', bins=100, label='2', alpha=1)
plt.hist(results_slices.loc[(results_slices['BIRADS'].isin(['4','5']))*(results_slices['y_true']==1)*(results_slices['min_distance'] == 3),'y_pred'], color='orange', bins=100, label='3', alpha=1)
plt.hist(results_slices.loc[(results_slices['BIRADS'].isin(['4','5']))*(results_slices['y_true']==1)*(results_slices['min_distance'] == 4),'y_pred'], color='yellow', bins=100, label='4', alpha=1)
plt.hist(results_slices.loc[(results_slices['BIRADS'].isin(['4','5']))*(results_slices['y_true']==1)*(results_slices['min_distance'] == 5),'y_pred'], color='gray', bins=100, label='5', alpha=1)


plt.vlines(x=results_slices.loc[(results_slices['BIRADS'].isin(['4','5']))*(results_slices['y_true']==1)*(results_slices['min_distance'] == 0),'y_pred'].min(),ymin=0,ymax=100,label='0', color='darkred', linestyle='--', linewidth=1)
plt.vlines(x=results_slices.loc[(results_slices['BIRADS'].isin(['4','5']))*(results_slices['y_true']==1)*(results_slices['min_distance'] == 1),'y_pred'].min(),ymin=0,ymax=100,label='1', color='red', linestyle='--', linewidth=1)
# plt.vlines(x=results_slices.loc[(results_slices['y_true']==1)*(results_slices['min_distance'] == 2),'y_pred'].min(),ymin=0,ymax=100,label='2', color='darkorange', linestyle='--', linewidth=1)
plt.vlines(x=results_slices.loc[(results_slices['BIRADS'].isin(['4','5']))*(results_slices['y_true']==1)*(results_slices['min_distance'] == 3),'y_pred'].min(),ymin=0,ymax=100,label='3', color='orange', linestyle='--', linewidth=1)
plt.vlines(x=results_slices.loc[(results_slices['BIRADS'].isin(['4','5']))*(results_slices['y_true']==1)*(results_slices['min_distance'] == 4),'y_pred'].min(),ymin=0,ymax=100,label='4', color='yellow', linestyle='--', linewidth=1)
plt.vlines(x=results_slices.loc[(results_slices['BIRADS'].isin(['4','5']))*(results_slices['y_true']==1)*(results_slices['min_distance'] == 5),'y_pred'].min(),ymin=0,ymax=100,label='5', color='gray', linestyle='--', linewidth=1)
